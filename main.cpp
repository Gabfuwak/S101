// projet S101
// Traitement d'images (et manipulation de vecteurs 2D)
...
// Prototypes des sous programmes personnels
void rotate90CW(string pathIn, string pathOut);
void mirrorH(string pathIn, string pathOut);
void mirrorV(string pathIn, string pathOut);
void lumMult(string pathIn, string pathOut, float LUMMULT = 1.5);
void toBinary(string pathIn, string pathOut, int SEUIL = 127);
void invertColors(string pathIn, string pathOut);
void removeRed(string pathIn, string pathOut);
void onlyRed(string pathIn, string pathOut);
void copyImage(string pathIn, string pathOut);
...
int main()
{
    ...
    rotate90CW("images/coco.ppm", "images/coco-90CW.ppm");

}


void copyImage(string pathIn, string pathOut){ //Q1

    vector<vector <int>>rouge,vert,bleu;

    loadPicture(pathIn, rouge, vert, bleu);
    writePicture(pathOut, rouge, vert, bleu);
}




void onlyRed(string pathIn, string pathOut){ //Q2

    vector<vector <int>>rouge,vert,bleu;

    loadPicture(pathIn, rouge, vert, bleu);
    for(int i = 0; i < vert.size(); i++){
        vert[i] = vector<int>(rouge.size(), 0);
        bleu[i] = vector<int>(bleu.size(), 0);
        }
...
void removeRed(string pathIn, string pathOut){ //Q3+4
    ...
    if(trouve){
        for(int i = 0; i < rouge.size(); i++){
            rouge[i] = vector<int>(rouge.size(), 0);
        }
...
void invertColors(string pathIn, string pathOut){ //Q5
...
void toBinary(string pathIn, string pathOut, int SEUIL){ //Q6
    ...
    for(int i = 0; i < rouge.size(); i++){
        for(int j = 0; j < rouge.size(); j++){
            if(((rouge[i][j] + vert[i][j] + bleu[i][j])/3) <= SEUIL){
                rouge[i][j] = 0;
                vert[i][j] = 0;
                bleu[i][j] = 0;
             }else{
...
void lumMult(string pathIn, string pathOut, float LUMMULT){ //Q7
    ...
    for(int i = 0; i < rouge.size(); i++){
        for(int j = 0; j < rouge.size(); j++){
            if( rouge[i][j]*LUMMULT > 255 )  {
                rouge[i][j] = 255;
            }else{
                rouge[i][j] =  rouge[i][j]*LUMMULT;
            }
            if( vert[i][j]*LUMMULT > 255 )  {
                vert[i][j] = 255;
            }else{
                vert[i][j] =  vert[i][j]*LUMMULT;
            }
            if( bleu[i][j]*LUMMULT > 255 )  {
                bleu[i][j] = 255;
            }else{
                bleu[i][j] =  bleu[i][j]*LUMMULT;
            }
        }
    }
...
///PARTIE 2 - TRANSFORMATIONS GEOMETRIQUES
...
void mirrorV(string pathIn, string pathOut){    //Q8 - 1 - axe vectical

    vector<vector <int>>rouge,vert,bleu;
    vector<vector <int>>nouvRouge,nouvVert,nouvBleu;

    loadPicture(pathIn, rouge, vert, bleu);

    for (int i = 0; i < rouge.size(); i++){ //Initialisation des nouveaux vecteurs
        nouvRouge.push_back(vector<int>(rouge.size(),0));
        nouvBleu.push_back(vector<int>(rouge.size(),0));
        nouvVert.push_back(vector<int>(rouge.size(),0));
    }
    for(int i = 0; i < nouvRouge.size(); i++){
        for(int j = 0; j < nouvRouge.size(); j++){

            nouvRouge[i][rouge.size()-j-1] = rouge[i][j];
            nouvVert[i][rouge.size()-j-1] = vert[i][j];
            nouvBleu[i][rouge.size()-j-1] = bleu[i][j];

        }
    }
    writePicture(pathOut, nouvRouge, nouvVert, nouvBleu);

}


void mirrorH(string pathIn, string pathOut){    //Q8 - 2 - axe horizontal
    ...
    for(int i = 0; i < nouvRouge.size(); i++){
        for(int j = 0; j < nouvRouge.size(); j++){
            nouvRouge[rouge.size()-i-1][j] = rouge[i][j];
            nouvVert[rouge.size()-i-1][j] = vert[i][j];
            nouvBleu[rouge.size()-i-1][j] = bleu[i][j];
        }
    }
    writePicture(pathOut, nouvRouge, nouvVert, nouvBleu);
...
void rotate90CW(string pathIn, string pathOut){
    ...
    for(int i = 0; i < nouvRouge.size(); i++){
        for(int j = 0; j < nouvRouge.size(); j++){
            nouvRouge[i][j] = rouge[j][rouge.size()-i-1];
            nouvVert[i][j] = vert[j][rouge.size()-i-1];
            nouvBleu[i][j] = bleu[j][rouge.size()-i-1];
        }
    }
    writePicture(pathOut, nouvRouge, nouvVert, nouvBleu);
